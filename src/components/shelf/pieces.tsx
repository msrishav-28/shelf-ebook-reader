...
import type { Work } from "@/lib/shelf-data";
...
export function CoverCard({ work, columns }: { work: Work; columns: 2 | 3 }) {
  return (
    <Link
      to="/series/$workId"
    ...
    >
      <div className="relative aspect-2/3">
        ...
        {work.unread > 0 ? (
          <span className="absolute top-1.5 left-1.5 rounded-full bg-badge px-1.5 py-0.5 text-[11px] leading-tight font-semibold text-badge-foreground">
            {work.unread}
          </span>
        ) : null}
...
export function ListRow({
  ...
  return (
    <button type="button" onClick={onClick} className="block w-full transition-colors hover:bg-muted/60">
      {inner}
    </button>
  );
}
